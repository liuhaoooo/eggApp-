...
class AliyunsmsService extends Service {
    ...
    async sendsms(PhoneNumbers, TemplateCode) {//发送短信验证码
        ...
        let isok = await service.aliyunsms.sms(smsClient, params);
        if (isok == "ok") {
            ...
            let res = await app.redis.set(`code_${PhoneNumbers}_${TemplateCode}`, code, 'EX', 60)//短信发送成功，将验证码存在redis
            return res == 'OK'
        } else {
            return false
        }
